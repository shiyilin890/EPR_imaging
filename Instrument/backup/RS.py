...
def runRS():
    ...
    Xepr.XeprCmds.vpSave('Current', 'Primary', title2, filename2)
    Xepr.XeprCmds.aqExpSelect(1,'Experiment')
    
    # Subtract Scan 2 from Scan 1
    
    Xepr.XeprCmds.vpLoad(filename1,'Primary')
    
    Xepr.XeprCmds.vpLoad(filename2,'Secondary')
...
def runRSgrad():
    ...
    dset2D1=Xepr.XeprDataset(size=(int(xPts),int(len(Garray))),xeprset = 'primary',iscomplex = True) # Create 2D dataset
    dset2D2=Xepr.XeprDataset(size=(int(xPts),int(len(Garray))),xeprset = 'secondary',iscomplex = True)
