...
def runRS():
    ...
    a1=hidden['NoOfAverages'].value	     # Get number of averages
    a2=hidden['NOnBoardAvgs'].value	     # Get number of on-board averages
    a3=hidden['TimeBase'].value	    	     # Get time base
    a4=hidden['NoOfPoints'].value            # Get number of points
    pts=int(a4)
...
def runRSgrad():
    ...
    Gmax= float(e301.get())
    Ng = int(2*Gmax/gstep+1)
    Garray=np.linspace(-Gmax, Gmax, Ng)
    #Garray=np.linspace(9.2, Gmax, 81)
    ...
    specr=np.zeros(( pts,Ng ))
    speci=np.zeros(( pts,Ng ))
	...
	### zero at high gradient   
	#if  ( abs(g+10)<0.6 ) or (abs(g-10)<0.6): #-9.4 ok, 9.4 error
	if  ( abs(g+10)<1 ) or (abs(g-10)<1):
	    ZGrad(0, 1)
...
eL108 = Label(frame1, text="Spectrum name: ")
